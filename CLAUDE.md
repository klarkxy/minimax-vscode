...
Thin wrapper around `@anthropic-ai/sdk`. [core.ts](src/client/core.ts) holds `MiniMaxClient` and the `ChatOptions` / request builder; [error.ts](src/client/error.ts) normalises transport / SDK errors into `MiniMaxRequestError` + `createUserFacingError` so the provider can surface them with action URLs.
...
- [consts.ts](src/consts.ts) — Compile-time constants (config section, default base URLs, secret/memento keys, default `~/.claude/projects` path, tool-call limit, replay-marker MIME, etc.). Anything depending on the VS Code runtime lives elsewhere.
- [i18n.ts](src/i18n.ts) — Zero-dep `t(key, ...args)` against a hand-written `en` / `zh-cn` dictionary. `package.nls.json` and `package.nls.zh-cn.json` mirror the command / config titles.
- [models/registry.ts](src/models/registry.ts) — Single source of truth for model definitions, capabilities (`imageInput`, `videoInput`, `toolCalling`, `thinking`), and pricing. `localizeModelPricing()` picks the CNY vs USD table from `minimax.apiBaseUrl`.

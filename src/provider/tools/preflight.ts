...
/**
 * Inspect the conversation for previous preflight rounds and decide whether
 * we still need to emit synthetic activate_* tool calls.
 *
 * The Copilot Chat host sometimes issues a tool list with placeholder
 * `activate_*` names that the model is expected to call once. Those calls
 * mutate the tool list to its real form. We synthesise them on the model's
 * behalf so the model never sees a different tool list between turns — that
 * would otherwise break the upstream prompt cache and tool flow.
 */
export function inspectActivatePreflight(
	...
	const remainingActivatorNames = activatorNames.filter(
		(name) => !calledActivatorNames.has(name),
	);
	return {
		rounds,
...
/**
 * Filter preflight tool calls and results from the request history so the
 * downstream provider sees a clean conversation. The synthetic preflight
 * parts are not part of the real semantic context.
 */
